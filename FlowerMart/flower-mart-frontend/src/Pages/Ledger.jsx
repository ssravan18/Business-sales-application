...
const Ledger = () => {
  ...
  const loadCustomerData = async (id) => {
    try {
      const balRes = await API.get(`/customer-balance/${id}`);
      setBalanceData(balRes.data);
      const histRes = await API.get(`/customer-history/${id}`);
  ...
  const handlePayment = async (e) => {
    ...
    try {
      await API.post('/payments', {
        ...
        transaction_type: 'CREDIT', // Customer giving money (Credit to us)
        method: 'CASH',
        date: payDate
      });
